...
const ServiceCard = ({ svc }) => {
  ...
  return (
    <motion.article
      className={`group relative flex flex-col justify-between overflow-hidden p-8 md:p-10 transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 hover:bg-[var(--color-lime)] hover:border-[var(--color-lime)] ${svc.span}`}
      style={{
    ...
    >
      ...
      <div
        className="absolute top-4 left-4 font-mono text-[8px] text-white/10 uppercase tracking-[0.2em] pointer-events-none group-hover:text-void/40 transition-colors"
        aria-hidden="true"
      >
        // SV_TYPE_{svc.num}
      </div>
      <div
        className="absolute bottom-4 right-4 font-mono text-[8px] text-white/10 uppercase tracking-[0.2em] pointer-events-none group-hover:text-void/40 transition-colors"
        aria-hidden="true"
      >
        COORD: {(Number(svc.num) * 13) % 90}.{svc.num}X
      </div>

      {/* Massive background number (moves slightly with mouse). Hollow stroke
          on the dark surface; flips to a solid void fill once the card
          inverts to lime, so it stays legible instead of disappearing. */}
      <motion.div
      ...
      >
        <span className="transition-all duration-500 [-webkit-text-stroke:1px_var(--color-lime)] text-transparent group-hover:[-webkit-text-stroke:0px] group-hover:text-[var(--color-void)]">
          {svc.num}
        </span>
      </motion.div>
      ...
      <div className="flex items-start justify-between relative z-10 mb-20 transition-all duration-500">
        <span className="font-mono text-[10px] tracking-[0.4em] uppercase text-white/40 group-hover:text-[var(--color-void)] transition-colors duration-500">
          {svc.sub}
        </span>
        ...
        <svg
          ...
          stroke="currentColor"
          strokeWidth="1.5"
          className="w-8 h-8 opacity-20 group-hover:opacity-100 group-hover:text-[var(--color-void)] transform transition-all duration-700 group-hover:rotate-180 group-hover:scale-110"
        >
      ...
      <div className="relative z-10">
        <h3
          className="font-black leading-none tracking-tighter mb-4 transition-all duration-500 origin-left group-hover:scale-[1.03] group-hover:text-[var(--color-void)] uppercase italic"
          style={{ fontSize: "clamp(2rem, 3.5vw, 3rem)" }}
        >
          {svc.title}
        </h3>

        <p className="text-sm md:text-base leading-relaxed opacity-40 group-hover:opacity-80 group-hover:text-[var(--color-void)] transition-all duration-500 max-w-sm">
          {svc.desc}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mt-8">
          {svc.tags.map((t) => (
            <span
              key={t}
              className="font-mono text-[10px] tracking-widest uppercase px-3 py-1.5 border border-white/5 group-hover:border-[var(--color-void)]/30 group-hover:text-[var(--color-void)] transition-all duration-500"
            >
              {t}
            </span>
          ))}
      ...
      {/* Side-loading progress line on hover — void so it reads against the
          inverted lime background instead of disappearing into it */}
      <div className="absolute left-0 bottom-0 h-0 w-[2px] bg-[var(--color-void)] transition-all duration-700 group-hover:h-full origin-bottom" />
    </motion.article>
  );
};
...
const Services = () => (
  <section
    id="services"
    className="relative min-h-screen py-32 px-6 md:px-12 lg:px-20 flex items-center"
      ...
      <div className="flex flex-col xl:flex-row xl:items-end justify-between mb-24 gap-12">
        <div className="max-w-4xl">
          ...
          >
            ...
            <br />
            <span
              className="opacity-30 italic font-mono tracking-tight lowercase"
        ...
        <p className="max-w-sm text-base md:text-lg leading-relaxed opacity-50 pb-2 border-l border-white/10 pl-8 font-mono">
          A'dan Z'ye marka kimliği inşasından, ince detaylı editöryal baskı
          süreçlerine kadar kapsamlı sanat yönetmenliği ve dijital strateji.
        </p>
      </div>
