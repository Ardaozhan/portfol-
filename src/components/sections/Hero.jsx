...
const SplitText = ({ text, className, style, delay = 0, accentLast = false }) => {
    const words = text.split(' ');
    return (
        <span className={`inline-flex flex-wrap gap-x-[0.22em] gap-y-0 ${className ?? ''}`} style={style}>
            {words.map((word, wi) => (
                <span key={wi} style={{ overflow: 'hidden', display: 'inline-block', lineHeight: 1 }}>
                    <motion.span
                        style={{
                            display: 'inline-block',
                            color: accentLast && wi === words.length - 1 ? 'var(--color-lime)' : undefined,
                        }}
                        initial={{ y: '110%', opacity: 0 }}
                        animate={{ y: '0%', opacity: 1 }}
                        transition={{ duration: 0.9, delay: delay + wi * 0.08, ease: [0.22, 1, 0.36, 1] }}
                    >
                        {word}
                    </motion.span>
                </span>
...
const TICKER_ITEMS = [
    'Kurumsal Kimlik', '·', 'Ambalaj Tasarımı', '·', 'Tipografi', '·',
    'Editöryal Tasarım', '·', 'Poster', '·', 'Marka Stratejisi', '·',
    'Yayın Tasarımı', '·', 'Broşür & Katalog', '·', 'Logo Tasarımı', '·',
    'Renk Teorisi', '·', 'Art Direction', '·', 'Sosyal Medya Görselleri', '·',
];
...
const Hero = () => {
    ...
    return (
        <section id="hero" className="relative min-h-screen flex flex-col justify-center px-6 md:px-12 lg:px-20 py-20 overflow-hidden" aria-label="Hero">
            ...
            {/* Edge Metadata */}
            <div className="absolute top-10 left-6 md:left-12 font-mono text-[9px] text-white/20 uppercase tracking-[0.4em] pointer-events-none">
                <p>Ref_PRT_2024</p>
                <p className="text-lime/40">Status: Active_Discovery</p>
            </div>
            <div className="absolute bottom-10 right-6 md:right-12 font-mono text-[9px] text-white/20 text-right uppercase tracking-[0.4em] pointer-events-none">
            ...
            <div className="relative z-10 flex flex-col gap-10 max-w-7xl">
                ...
                >
                    <div className="flex gap-1">
                        {[1, 2, 3].map(i => <div key={i} className="w-1 h-1 bg-lime/40" />)}
                    </div>
                ...
                {/* Display Heading */}
                <h1 className="font-black tracking-tighter leading-[0.82] select-none" style={{ fontSize: 'clamp(4rem, 14vw, 13rem)' }}>
                ...
                <div className="grid md:grid-cols-12 gap-12 items-end">
                    ...
                    >
                        <p className="text-lg md:text-xl text-white/50 leading-relaxed font-light">
                            Markaları <span className="text-white">görsel sistemlere</span> dönüştürüyorum.
                            Analitik bir yaklaşımla, her dökümanı birer anlam katmanına işliyorum.
                        </p>
                    </motion.div>
            ...
            >
                <div className="text-[40vw] font-black leading-none uppercase select-none">AR_D</div>
            </motion.div>
        </section>
    );
};
