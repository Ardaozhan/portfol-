...
const CHAPTERS = [
    {
        ...
        body:
            'İstanbul merkezli bir grafik tasarımcı ve art director olarak kurumsal kimlikten ambalaj tasarımına, editöryal yayından dijital görsel sistemlere kendi imzamı taşıyan çalışmalar üretiyorum. Benim için tasarım, sadece güzellik değil — doğru mesajın doğru formu.',
        accent: '#e8e8e8',
    ...
    },
    {
        ...
        body:
            'Tasarıma kağıt ve mürekkeple başladım — elle çizim, baskı atölyeleri, ofset makinelerinin kokusu. Sonra piksele geçtim. Her medium aynı soruyu soruyordu: Bu yüzey nasıl bir şey hissettirmeli?',
        accent: '#ffb347',
    ...
    },
    {
        ...
        body:
            'Adobe Illustrator · InDesign · Photoshop · Figma · After Effects · Blender · Procreate · Glyphs · Pantone · Ofset Baskı. Araçlar değişir. Detaya olan takıntı değişmez.',
        accent: '#7eb8f7',
    ...
    },
    {
        ...
        body:
            'Her seçilen font, bir cümledir. Her renk farkı, bir duygu kararıdır. Her boşluk, bilinçli bir hiyerarşi kurulumudur. Dikkat eden insanlar için dikkat ederek tasarım yapıyorum.',
        accent: '#c4b5fd',
...
const FloatingPhoto = ({ chapter }) => {
    ...
    const translateX = useSpring(useMotionValue(0), { stiffness: 60, damping: 18 });
    const translateY = useSpring(useMotionValue(0), { stiffness: 60, damping: 18 });
    ...
    return (
        ...
        >
            ...
            >
                ...
                <p
                    className="font-black text-xl leading-none text-white tracking-tighter"
                >
                    {chapter.imgTitle}
                </p>
            </div>
                    ...
                    {chapter.id === 'skills' ? (
                        <div className="relative py-4 overflow-hidden mask-fade-right">
                            <motion.div
                                ...
                                style={{ x: useTransform(velocity, [0, 1], [0, -200]) }}
                            >
                                {chapter.body.split(' · ').map((skill, si) => (
                                    <span key={si} className="font-mono text-xs border border-white/20 px-4 py-2 rounded-full text-white/40 group-hover:text-white group-hover:border-lime transition-all">
                                        {skill}
                                    </span>
                                ))}
                            </motion.div>
                            <motion.div
                                ...
                                style={{ x: useTransform(velocity, [0, 1], [-200, 0]) }}
                            >
                                {[...chapter.body.split(' · ')].reverse().map((skill, si) => (
                                    <span key={si} className="font-mono text-xs border border-white/20 px-4 py-2 rounded-full text-white/40">
                                        {skill}
                                    </span>
                                ))}
                            </motion.div>
