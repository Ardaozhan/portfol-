...
const InputField = ({
  ...
  <div className="flex flex-col gap-2">
    ...
    >
      {label}{" "}
      {required && <span style={{ color: "var(--color-lime)" }}>*</span>}
    </label>
    {type === "textarea" ? (
      <textarea
        name={name}
        ...
        onFocus={(e) =>
          (e.target.style.borderBottomColor = "var(--color-lime)")
        }
        onBlur={(e) =>
...
const Contact = () => {
  ...
  const handleSubmit = useCallback(
    (e) => {
      ...
      // No backend/email service is wired up yet, so the most honest way to
      // actually deliver the message is to hand off to the user's own email
      // client, pre-filled with their form data.
      const subject = encodeURIComponent(
        `Yeni Proje Talebi${form.type ? ` — ${form.type}` : ""}`,
      );
      const body = encodeURIComponent(
        `İsim: ${form.name}\nE-Posta: ${form.email}\nProje Türü: ${form.type || "-"}\n\nMesaj:\n${form.message}`,
      );
      window.location.href = `mailto:merhaba@ardao.design?subject=${subject}&body=${body}`;

      // Small delay so the hand-off has a moment to happen before the UI
  ...
  return (
    ...
    >
      {/* SOL: Büyük CTA */}
      <div
        className="relative flex flex-col justify-between p-8 md:p-14 md:w-1/2"
      ...
      >
        <div>
          ...
          <p
            ...
            style={{ color: "var(--color-muted)" }}
          >
            Yeni marka projeleri, ambalaj tasarımı, editöryal çalışmalar ve
            sanat yönetmenliği için açığım. Genellikle{" "}
            <span style={{ color: "var(--color-chalk)" }}>24 saat</span> içinde
            dönüş yapıyorum.
          </p>
        </div>
        ...
        <span
          className="absolute bottom-4 right-4 font-black pointer-events-none select-none"
          style={{
            fontSize: "clamp(5rem,12vw,10rem)",
            color: "rgba(212,245,0,0.03)",
            lineHeight: 1,
          }}
          aria-hidden="true"
        >
          ...
          {sent ? (
            ...
            >
              ...
              <p
                ...
                style={{ color: "var(--color-muted)" }}
              >
                Göndermek için son adımı e-posta uygulamanızda tamamlayın.
              </p>
            </motion.div>
          ) : (
            <motion.form
            ...
            >
              ...
              <InputField
                ...
                value={form.type}
                onChange={handleChange}
                placeholder="Kurumsal Kimlik / Ambalaj / Editöryal / Diğer"
              />
              <InputField
                label="Mesajınız"
                name="message"
